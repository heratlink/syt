...
const requestHOS = axios.create({
  // 公共配置，将来发送请求会自动添加
  // 发送请求的前缀：将来所有通过request发送的请求，会在请求地址前面自动添加baseURL
  // react脚手架会根据开发/生产自动加载.env.xxx的环境变量
  // 通过 process.env.xxx 读取加载的环境变量的值
  // 开发模式下：process.env.REACT_APP_API --> /hospital
  baseURL: process.env.REACT_APP_HOS_API, // 请求地址：baseURL + url
  timeout: 10000, // 请求超时时间：一旦请求超过10s还未完成，就会自动中断请求
  // headers: {},
});

// 请求拦截器函数：发送请求之前触发的函数
requestHOS.interceptors.request.use((config) => {
  // token从redux中读取
  // 不是组件，不能使用hooks函数读取
  // 使用最原始读取redux数据的方法
...
// 响应拦截器函数：得到响应之后触发的函数
requestHOS.interceptors.response.use(
  (response) => {
    // 请求成功
